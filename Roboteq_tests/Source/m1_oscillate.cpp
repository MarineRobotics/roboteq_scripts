...
int MotorOscillate(string port)
{
    	cout << endl << "Motor Oscillate:" << endl;
	cout << "------------------------" << endl;
	string response = "";
	...
	// Set motor to left position
	cout << "- Motor Left Position: SetCommand(_G, 1, -1000)...";
    ...
    // Set motor to right position
    cout << "- Motor Right Position: SetCommand(_G, 1, 1000)...";
    end = std::chrono::steady_clock::now() + std::chrono::seconds(3);
    while (std::chrono::steady_clock::now() < end) {
        if ((status = device.SetCommand(_G, 1, 1000)) != RQ_SUCCESS)
            cout << "failed --> " << status << endl;
        else
    ...
    // Set motor to center position
    cout << "- Motor Center Position: SetCommand(_G, 1, 0)...";
    end = std::chrono::steady_clock::now() + std::chrono::seconds(3);
    while (std::chrono::steady_clock::now() < end) {
